...
// Golden tests: compile Peter Schranz's actual PLD sources and prove
// functional equivalence with the fuse maps produced by WinCUPL 5.0a and
// GALasm 2.1 — the same JEDs running in real DCJ11 SBC hardware.
...
@Suite struct PLDParserTests {
    ...
    @Test func parsesV133WithOEExtension() throws {
        let d = try PLDParser.parse(try fixtureText("DCJ11SBC-V1-3-3", ext: "PLD"))
...
@Suite struct GoldenCompilation {
    ...
    @Test func v133MatchesGALasm() throws {
        // V1-3-3 uses IO.OE = 'b'0 — must select complex mode and match the
        // GALasm-built image on every output. Pin 18's *logic* is allowed to
        // differ (GALasm kept a documentation-only term behind the disabled
        // driver; we compile the source's 'b'0 faithfully).
        let compiled = try PLDCompiler.compile(try fixtureText("DCJ11SBC-V1-3-3", ext: "PLD"))
        let golden = try fixture("DCJ11SBC-V1-3-3-galasm")
        ...
        let diff = try FuseDiff.gal16v8(compiled.jed, golden)
        for pin in diff.pins {
            #expect(pin.oeEquivalent, "pin \(pin.pin) OE differs from GALasm")
            if pin.pin != 18 {
                #expect(pin.logicEquivalent, "pin \(pin.pin) logic differs from GALasm")
            }
        }
    }
    ...
    @Test func crossVersionEquivalence() throws {
        ...
        let b = try PLDCompiler.compile(try fixtureText("DCJ11SBC-V1-3-3", ext: "PLD")).jed
